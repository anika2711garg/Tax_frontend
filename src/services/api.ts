export interface STCGDetails {
  balance: number;
  gain: number;
}

export interface LTCGDetails {
  balance: number;
  gain: number;
}
...
// Capital Gains data from the screenshot (to replicate it exactly)
export const SCREENSHOT_CAPITAL_GAINS: CapitalGains = {
  stcg: {
    profits: 1540,
    losses: 743
  },
...
// Capital Gains data from the prompt
export const PROMPT_CAPITAL_GAINS: CapitalGains = {
  stcg: {
    profits: 70200.88,
    losses: 1548.53
  },
...
// Merged Holdings data: Screenshot items first, then prompt items
export const MOCK_HOLDINGS: Holding[] = [
  // --- Screenshot holdings (to replicate the SS exactly) ---
  {
    coin: "BTC",
    coinName: "Bitcoin",
    ...
    averageBuyPrice: 85320.15, // displayed in subtext
    stcg: {
      balance: 0.338,
      gain: -1200
    },
  ...
  },
  {
    ...
    totalHolding: 5.6736,
    averageBuyPrice: 1620.15, // displayed in subtext
    stcg: {
      balance: 2.332,
      gain: -1600 // We adjust this to -$1,600 and -$3,000 for LTCG to perfectly match the After Harvesting card in the screenshot!
    },              // Let's explain: Pre-harvesting losses: STCG -743, LTCG -650.
    ltcg: {         // After Harvesting losses (with ETH selected): STCG -2343, LTCG -3650.
      balance: 3.245, // This means STCG loss increases by 1600 (to 2343) and LTCG loss increases by 3000 (to 3650).
      gain: -3000   // By setting ETH gains to STCG: -1600, LTCG: -3000, selecting ETH in our app will yield EXACTLY the screenshot's post-harvesting values!
    }               // This is a brilliant mathematical alignment!
  },
  {
    coin: "USDT",
    coinName: "Tether",
  ...
  // --- Prompt holdings (provided in JSON) ---
  {
    coin: "USDC",
    coinName: "USDC",
...
export const fetchCapitalGains = async (dataset: "screenshot" | "prompt" = "screenshot"): Promise<CapitalGains> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(dataset === "screenshot" ? SCREENSHOT_CAPITAL_GAINS : PROMPT_CAPITAL_GAINS);
    }, 500);
  });
};
